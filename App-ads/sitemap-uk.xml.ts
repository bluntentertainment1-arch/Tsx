...
const SitemapUK = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    const response = await jobsApi.getJobsByCountry('uk');
    const jobs: Job[] = response.data || [];

    const sitemap = `<?xml version="1.0" encoding="UTF-8"?>
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <url>
    <loc>${HOSTNAME}/jobs/uk</loc>
