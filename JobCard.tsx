  ...
  return (
    <>
      <div 
        ...
        className={`card p-6 hover:scale-[1.02] transition-all cursor-pointer ${isFeatured ? 'border-2 border-amber-400 bg-gradient-to-br from-amber-50 to-yellow-50' : ''}`}
      >
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          >
            <i className={`fa ${isSaved ? 'fa-bookmark' : 'fa-bookmark-o'} text-xl ${isSaved ? 'text-blue-600' : 'text-blue-600'}`}></i>
          </button>
        </div>

        <p className="text-slate-700 text-sm mb-4 line-clamp-3">{job.description}</p>
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          >
            <i className={`fa ${job.visa_tag === 'Visa Sponsored' ? 'fa-check-circle' : 'fa-info-circle'} mr-1`}></i>
            {job.visa_tag}
          </span>
        </div>
      </div>
